...
export default {
	async fetch(request, env, ctx): Promise<Response> {
		...
		if (url.pathname === '/api/generate/prompt' && request.method === 'POST') {
			...
			if (response && response.output) {
				...
				if (aiMessage && Array.isArray(aiMessage.content)) {
					imagePormpt = aiMessage.content.find((c) => c.type === 'output_text')?.text ?? '';
				}
			}
			return new Response(JSON.stringify({ prompt: imagePormpt }), {
				status: 200,
		...
		if (url.pathname === '/api/generate/image' && request.method === 'POST') {
			...
			const generateImage = await env.AI.run('@cf/leonardo/lucid-origin', {
				prompt: prompt,
				num_steps: 3,
			});
